...
**For lesion studies**  
Loads a network that was trained sequentially on shape->color->number with new expert network added in each sort rule (n1 &larr; n2->n3), then gating network (dnet) was trained on classic interleaved WCST with all experts present.  
